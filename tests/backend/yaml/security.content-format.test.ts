...
describe('Content format guards', () => {
  it('rejects JSON content even with .yaml extension', async () => {
    ...
    const res = await validateYaml(json, { filename: 'file.yaml', mimeType: 'application/yaml', toolRunner: noopRunner as any })
  ...
  it('rejects likely-binary content', async () => {
    ...
    const res = await validateYaml(binary, { filename: 'file.yml', mimeType: 'text/yaml', toolRunner: noopRunner as any })
    expect(res.ok).toBe(false)
  ...
  it('accepts plain YAML', async () => {
    const yaml = 'foo: bar\nlist:\n  - 1\n  - 2\n'
    const res = await validateYaml(yaml, { filename: 'ok.yaml', mimeType: 'application/yaml', toolRunner: noopRunner as any })
