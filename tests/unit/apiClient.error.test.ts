...
const ok = (data: unknown, init?: Partial<ResponseInit>) =>
...
const err = (status: number, data?: unknown) =>
  new Response(JSON.stringify(data ?? { error: "x" }), {
    status,
    headers: { "Content-Type": "application/json" },
  });
...
describe("apiClient error handling", () => {
  ...
  it("validate throws ApiError on 400/500", async () => {
    ...
    (fetch as unknown as MockedFunction<FetchMock>).mockImplementation(
      async () => err(500)
    );
  ...
  it("applySuggestions throws ApiError", async () => {
    ...
    await expect(api.applySuggestions("x", [0])).rejects.toMatchObject({
      status: 422,
    });
  });
  ...
  it("convert throws ApiError", async () => {
    ...
    await expect(api.convert({ yaml: "x" })).rejects.toMatchObject({
      status: 400,
    });
  });
